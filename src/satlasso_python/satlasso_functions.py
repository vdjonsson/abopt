...
def compute_gradient_satlasso_loss(beta, Xu, Xs, yu, ys, lmbdas):
    ...
    if np.all(np.add(ys, -1*np.matmul(Xs,beta)) < 0):
        ...
        sat_grad = [0]*len(beta)
        sat_grad[max_element] = np.sum(-1*np.array(Xs),axis=0)[max_element]
        
    zero_indices = [i for i in range(0,len(beta)) if beta[i]==0]
    l1_grad = np.sign(beta)
    l1_grad[zero_indices] = np.random.uniform(low=-1,high=1, size=len(zero_indices))
...
def gradient_descent(initial_beta, Xu, Xs, yu, ys, alpha, lmbdas, tol):
    while(True):
        beta = np.sum(initial_beta, -1*alpha*compute_gradient_satlasso_loss(initial_beta, Xu, Xs, yu, ys, lmbdas)).tolist()
        if(np.linalg.norm(np.add(beta, -1*np.array(initial_beta)), ord=2) < tol):
            break
        initial_beta = beta
    return beta

def satlasso(Xu, Xs, yu, ys, alpha, lmbdas):
    # Xu - unsaturated data
    # Xs - saturated data
...
def cross_validation(Xu, Xs, yu, ys, alphas, lmbda1_vals, lmbda2_vals, lmbda3_vals, num_folds):
    # Xu - unsaturated data
    # Xs - saturated data
    ...
    param_combns = np.array(np.meshgrid(alphas, lmbda1_vals, lmbda2_vals, lmbda3_vals)).T.reshape(-1,3)
    mses = []
    for i in range(0, len(param_combns)):
        ...
        for j in range(0, num_folds):
            ...
            beta = satlasso(train_data_Xu, train_data_Xs, train_data_yu, train_data_ys, alpha, lmbda_combn)
...
def satlasso_solve(X, y, alphas, lmbda1_vals, lmbda2_vals, lmbda3_vals, num_folds):
    # X - data
    # y - labels
    # lmbda1_vals - potential values for lambda1
    # lmbda2_vals - potential values for lambda2
    ...
    optimal_alpha, optimal_lmbdas = cross_validation(Xu, Xs, yu, ys, alphas, lmbda1_vals, lmbda2_vals, lmbda3_vals, num_folds)
